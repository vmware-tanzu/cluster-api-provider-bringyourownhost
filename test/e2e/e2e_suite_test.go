...
import (
	...
	infraproviderv1 "github.com/vmware-tanzu/cluster-api-provider-byoh/apis/infrastructure/v1beta1"
...
func setupSpecNamespace(caseContextData *CaseContext, artifactFolder string) {
	ctx := caseContextData.ctx
	clusterProxy := caseContextData.clusterProxy
	specName := caseContextData.CaseName

	Byf("Creating a namespace for hosting the %q test spec", specName)
	caseContextData.Namespace, caseContextData.cancelWatches = framework.CreateNamespaceAndWatchEvents(ctx, framework.CreateNamespaceAndWatchEventsInput{
...
func dumpSpecResourcesAndCleanup(caseContextData *CaseContext, artifactFolder string, intervalsGetter func(spec, key string) []interface{}, skipCleanup bool) {
	ctx := caseContextData.ctx
	cluster := caseContextData.ClusterResources.Cluster
	clusterProxy := caseContextData.clusterProxy
	namespace := caseContextData.Namespace
	cancelWatches := caseContextData.cancelWatches
	specName := caseContextData.CaseName
