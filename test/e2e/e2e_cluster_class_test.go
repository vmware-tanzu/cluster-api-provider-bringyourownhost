...
var _ = Describe("When BYOH joins existing cluster [Cluster-Class]", func() {
	...
	var (
		...
		byoHostName1        = "byohostcc1"
		byoHostName2        = "byohostcc2"
	)
	...
	It("Should create a workload cluster with single BYOH host for cluster class", func() {
		...
		Showf("cluster template %+v", string(workloadClusterTemplate))
		cpip:= os.Getenv("CONTROL_PLANE_ENDPOINT_IP")
		cniResource:= os.Getenv("CNI_RESOURCES")

		Showf("control plane end point  %+v", cpip)
		Showf("CNI_RESOURCES  %+v", cniResource)
