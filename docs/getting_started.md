...
To initialize Cluster API Provider vSphere, clusterctl requires the following settings, which should
be set in `~/.cluster-api/clusterctl.yaml` as the following:

``` yaml
providers:
  - name: byoh
    url: https://github.com/yixingjia/byoh/releases/latest/infrastructure-components.yaml
...
...
byoh           InfrastructureProvider   https://github.com/yixingjia/byoh/releases/latest/                                           infrastructure-components.yaml
...
...
1. Download the agent-linux-amd64 
2. Save the management cluster kubeconfig file as management.conf
3. Start the agent 
```shell
./agent-linux-64 -kubeconfig management.conf > agent.log 2>&1 &
```
